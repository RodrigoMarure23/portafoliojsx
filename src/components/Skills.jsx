...
function SkillCard({
  ...
  return <motion.div ref={ref} initial={{
    ...
    delay: index * 0.1
      ...
      <div className={`inline-flex p-3 rounded-lg bg-gradient-to-br ${colorClasses[category.color]} mb-4`}>
      ...
      <div className="space-y-3">
        {category.skills.map((skill, i) => <div key={skill.key}>
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm text-slate-400 font-mono">{t(`skills.categories.${category.key}.skills.${skill.key}`)}</span>
          ...
          }} className={`h-full bg-gradient-to-r ${colorClasses[category.color]} rounded-full`}></motion.div>
            </div>
          </div>)}
      </div>
    </motion.div>;
...
export function Skills() {
  ...
  return <section id="skills" className="py-24 bg-[#0a0e27] relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        ...
        duration: 0.6
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-white font-mono">
            {t('skills.title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">{t('skills.titleHighlight')}</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, index) => <SkillCard key={category.key} category={category} index={index} />)}
        </div>
      </div>
    </section>;
}